...
function WasteCategory() {
    ...
    const fetchCategories = async (searchQuery = '') => {
        setLoading(true);
        try {
            const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/c/categorycollector`, {
    ...
    const getCategoryInfo = (name) => {
        ...
        if (lower.includes('ขยะเปื้อน')) return { image: '/images/logo192.jpg', link: '/c/dirtywastecollector', color: '#EAB308' }; // Yellow
        if (lower.includes('วัสดุรีไซเคิล')) return { image: '/images/recyclable.jpg', link: '/c/sellwastecollector', color: '#43A047' }; // Green
        if (lower.includes('ขยะย่อยสลาย')) return { image: '/images/compostable.jpg', link: '/c/composablewastecollector', color: '#8BC34A' }; // Light Green
        if (lower.includes('ขยะพลังงาน')) return { image: '/images/energy.jpg', link: '/c/energyrdfwastecollector', color: '#FB8C00' }; // Orange
        if (lower.includes('ขยะอันตราย')) return { image: '/images/hazardous.jpeg', link: '/c/hazardouswastecollector', color: '#E53935' }; // Red
        if (lower.includes('ขยะห้องน้ำ')) return { image: '/images/bathroom.jpg', link: '/c/bathroomwastecollector', color: '#039BE5' }; // Light Blue
        if (lower.includes('ขยะชิ้นใหญ่')) return { image: '/images/big.jpg', link: '/c/bigwastecollector', color: '#5E35B1' }; // Purple
        
        // Sub-categories
        if (lower.includes('พลาสติก')) return { image: '/images/plastic.jpg', link: '/c/sellwastecollector/plastic', color: '#1E88E5' };
        if (lower.includes('กระดาษ')) return { image: '/images/paper.jpg', link: '/c/sellwastecollector/paper', color: '#795548' };
        if (lower.includes('ขวดแก้ว')) return { image: '/images/glass.jpg', link: '/c/sellwastecollector/glass', color: '#43A047' };
        if (lower.includes('โลหะ')) return { image: '/images/metal.jpg', link: '/c/sellwastecollector/metal', color: '#607D8B' };
            ...
            <Box sx={{ 
                ...
                <Container maxWidth="md">
                    ...
                    <Typography variant="h6" sx={{ fontFamily: 'Sarabun', opacity: 0.9, fontWeight: 300 }}>
                        เรียนรู้การแยกขยะแต่ละประเภทอย่างถูกต้อง เพื่อสิ่งแวดล้อมที่ยั่งยืน
                    </Typography>
                </Container>
            </Box>

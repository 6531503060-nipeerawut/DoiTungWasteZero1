...
function WasteCategory() {
    ...
    const fetchCategories = async (searchQuery = '') => {
        setLoading(true);
        try {
            const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/v/categoryvillager`, {
    ...
    const getCategoryInfo = (name) => {
        ...
        if (lower.includes('ขยะเปื้อน')) return { image: '/images/logo192.jpg', link: '/v/dirtywastevillager', color: '#EAB308' }; // Yellow
        if (lower.includes('วัสดุรีไซเคิล')) return { image: '/images/recyclable.jpg', link: '/v/sellwastevillager', color: '#43A047' }; // Green
        if (lower.includes('ขยะย่อยสลาย')) return { image: '/images/compostable.jpg', link: '/v/composablewastevillager', color: '#8BC34A' }; // Light Green
        if (lower.includes('ขยะพลังงาน')) return { image: '/images/energy.jpg', link: '/v/energyrdfwastevillager', color: '#FB8C00' }; // Orange
        if (lower.includes('ขยะอันตราย')) return { image: '/images/hazardous.jpeg', link: '/v/hazardouswastevillager', color: '#E53935' }; // Red
        if (lower.includes('ขยะห้องน้ำ')) return { image: '/images/bathroom.jpg', link: '/v/bathroomwastevillager', color: '#039BE5' }; // Light Blue
        if (lower.includes('ขยะชิ้นใหญ่')) return { image: '/images/big.jpg', link: '/v/bigwastevillager', color: '#5E35B1' }; // Purple
        
        // Sub-categories
        if (lower.includes('พลาสติก')) return { image: '/images/plastic.jpg', link: '/v/sellwastevillager/plastic', color: '#1E88E5' };
        if (lower.includes('กระดาษ')) return { image: '/images/paper.jpg', link: '/v/sellwastevillager/paper', color: '#795548' };
        if (lower.includes('ขวดแก้ว')) return { image: '/images/glass.jpg', link: '/v/sellwastevillager/glass', color: '#43A047' };
        if (lower.includes('โลหะ')) return { image: '/images/metal.jpg', link: '/v/sellwastevillager/metal', color: '#607D8B' };

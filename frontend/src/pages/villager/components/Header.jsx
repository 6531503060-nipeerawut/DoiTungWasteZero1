...
function Header({ type = 'default', villId }) {
    ...
    const navLinks = [
        { to: '/v/wastepricevillager', label: 'ราคารับซื้อ' },
        { to: '/v/categoryvillager', label: 'ประเภทขยะ' },
        { to: '/v/garbagetruckschedulevillager', label: 'ตารางรถเก็บขยะ' },
        { to: '/carbons', label: 'คำนวณคาร์บอน' },
        { to: `/v/profile-villager/${villId}`, label: 'บัญชีผู้ใช้' }
    ];

    const renderNavbar = () => (
        <nav className="navbar navbar-expand-lg navbar-light" style={{ background: 'linear-gradient(135deg, #a1c4fd, #c2e9fb)' }}>
            <Link className="navbar-brand" to="/v/homevillager">

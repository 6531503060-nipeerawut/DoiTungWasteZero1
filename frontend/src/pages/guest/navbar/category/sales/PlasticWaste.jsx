...
import { 
    Inventory2 as PlasticIcon, // ใช้ไอคอนกล่อง/พลาสติก
    ArrowBackIos, 
    Close as CloseIcon,
    Scale as ScaleIcon
...
function PlasticWaste() {
    ...
    // Image Mapping
    const getPlasticImage = (name) => {
        if (!name) return '/images/assets/glass-default.png';
        if (name.includes('ขวดพลาสติก PET ใส')) return '/images/assets/plasticwaste-1.png';
        if (name.includes('ขวดพลาสติกขุ่น')) return '/images/assets/plasticwaste-2.png';
        if (name.includes('พลาสติกกรอบ')) return '/images/assets/plasticwaste-3.png';
        if (name.includes('ท่อ PVC')) return '/images/assets/plasticwaste-4.png';
        if (name.includes('ช้อนไฟเบอร์')) return '/images/assets/plasticwaste-5.png';
        if (name.includes('พลาสติกดำ')) return '/images/assets/plasticwaste-6.png';
        if (name.includes('ช้อนพลาสติก')) return '/images/assets/plasticwaste-7.png';
        if (name.includes('เชือกพลาสติกแพ็คกล่อง')) return '/images/assets/plasticwaste-8.png';
        if (name.includes('พลาสติกรวม')) return '/images/assets/plasticwaste-13.png';
        if (name.includes('ถุงพลาสติกสะอาด')) return '/images/assets/plasticwaste-9.png';
        if (name.includes('เสื่อน้ำมัน')) return '/images/assets/plasticwaste-10.png';
        if (name.includes('กระสอบฟาง')) return '/images/assets/plasticwaste-11.png';
        if (name.includes('สายไฟฟ้า')) return '/images/assets/plasticwaste-14.png';
        if (name.includes('พลาสติกเนื้อไฟเบอร์')) return '/images/assets/plasticwaste-12.png';
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/sellwaste/plastic`);
        ...
        <div style={{ backgroundColor: '#F8F9FA', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <Box sx={{ bgcolor: themeColor, color: 'white', py: 5, textAlign: 'center', mb: 4, boxShadow: 3 }}>
                <Container>
                    <Typography variant="h4" fontWeight="bold" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1.5 }}>
                        <PlasticIcon fontSize="large" /> คำนวณราคาพลาสติก
                    </Typography>
                    <Typography variant="body1" sx={{ mt: 1, opacity: 0.9 }}>
                        เลือกประเภทพลาสติกเพื่อคำนวณราคารับซื้อ
                    </Typography>
                </Container>
            </Box>
            ...
            <Container maxWidth="lg" sx={{ flexGrow: 1, mb: 8 }}>
                ...
                <Box sx={{ mb: 4 }}>
                    <Button 
                        ...
                        onClick={handleBack} 
                        sx={{ color: themeColor, fontWeight: 'bold', '&:hover': { bgcolor: `${themeColor}15` } }}
                    >
                        กลับไปหน้ารับซื้อ
                    </Button>
                </Box>
                                    ...
                                    >
                                        <CardActionArea onClick={() => handleItemClick(item)} sx={{ p: 2, textAlign: 'center' }}>
                                            ...
                                            <Box sx={{ height: 180, display: 'flex', alignItems: 'center', justifyContent: 'center', mb: 2 }}>
                                                <img 
                                                    src={getPlasticImage(item.item_name)} 
                                                    alt={item.item_name} 
            ...
            <Dialog open={openDialog} onClose={handleCloseDialog} fullWidth maxWidth="xs" PaperProps={{ sx: { borderRadius: 4 } }}>
                    ...
                    {selectedItem && (
                        <>
                            ...
                            <Box sx={{ height: 200, display: 'flex', justifyContent: 'center', mb: 2, mt: 2 }}>
                                <img 
                                    src={getPlasticImage(selectedItem.item_name)} 
                            ...
                            <Paper elevation={0} sx={{ bgcolor: '#E3F2FD', p: 2, borderRadius: 3, border: `1px dashed ${themeColor}` }}>
                ...
                <DialogActions sx={{ p: 2, justifyContent: 'center' }}>
                    <Button onClick={handleCloseDialog} variant="contained" fullWidth sx={{ bgcolor: themeColor, borderRadius: 2, py: 1.2, fontSize: '1rem', '&:hover': { bgcolor: '#1976D2' } }}>
                        เสร็จสิ้น
                    </Button>
                </DialogActions>
            </Dialog>

...
const themeColor = '#4CAF50'; // สีเขียวสำหรับแก้ว
...
function GlassWaste() {
    ...
    // Image Mapping
    const getGlassImage = (name) => {
        if (!name) return '/images/assets/glass-default.png';
        if (name.includes('ใส')) return '/images/assets/glass-clear.png';
        if (name.includes('น้ำตาล')) return '/images/assets/glass-brown.png';
        if (name.includes('เขียว')) return '/images/assets/glass-green.png';
        if (name.includes('แก้วแตก')) return '/images/assets/glass-broken.png';
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/sellwaste/glass`);
        ...
        <div style={{ backgroundColor: '#F8F9FA', minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <Box sx={{ bgcolor: themeColor, color: 'white', py: 5, textAlign: 'center', mb: 4, boxShadow: 3 }}>
                <Container>
                    <Typography variant="h4" fontWeight="bold" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1.5 }}>
                        <GlassIcon fontSize="large" /> คำนวณราคาขวดแก้ว
                    </Typography>
                    <Typography variant="body1" sx={{ mt: 1, opacity: 0.9 }}>
                        เลือกประเภทขวดแก้วเพื่อคำนวณราคารับซื้อ
                    </Typography>
                </Container>
            </Box>
            ...
            <Dialog open={openDialog} onClose={handleCloseDialog} fullWidth maxWidth="xs" PaperProps={{ sx: { borderRadius: 4 } }}>
                ...
                <DialogContent sx={{ pt: 4, pb: 2, textAlign: 'center' }}>
                    {selectedItem && (
                        <>
                            {/* ✅ 2. ปรับรูปใน Modal ให้ใหญ่ขึ้น */}
                            <Box sx={{ height: 200, display: 'flex', justifyContent: 'center', mb: 2, mt: 2 }}>
                                <img 
                                    src={getGlassImage(selectedItem.item_name)} 
                            ...
                            <Paper elevation={0} sx={{ bgcolor: '#E8F5E9', p: 2, borderRadius: 3, border: `1px dashed ${themeColor}` }}>
                ...
                <DialogActions sx={{ p: 2, justifyContent: 'center' }}>
                    <Button onClick={handleCloseDialog} variant="contained" fullWidth sx={{ bgcolor: themeColor, borderRadius: 2, py: 1.2, fontSize: '1rem' }}>
                        เสร็จสิ้น
                    </Button>
                </DialogActions>
            </Dialog>
